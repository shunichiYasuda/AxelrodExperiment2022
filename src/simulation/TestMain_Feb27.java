...
public class TestMain_Feb27 {
	...
	public static void main(String[] args) {
		...
		// 記憶パターンからみた「お人よし」個体数の記録
		int[] memBasedYesMan = new int[GEN];
		// 記憶パターンからみた「お人好し」個体数を EXP分保存するテーブル
		int[][] memBasedYesManTable = new int[GEN][EXP];
		// 記憶パターンからみた「裏切り者」個体数の記録
		int[] memBasedTraitor = new int[GEN];
		// 記憶パターンからみた「裏切り者」個体数を EXP分保存するテーブル
		int[][] memBasedTraitorTable = new int[GEN][EXP];
		// 記憶パターンからみた「あまのじゃく」個体数の記録
		int[] memBasedContrary = new int[GEN];
		// 記憶パターンからみた「あまのじゃく」個体数を EXP分保存するテーブル
		int[][] memBasedContraryTable = new int[GEN][EXP];
		...
		// 個体数を記録するテーブルの初期化
		initTableInt(memBasedTFTTable);
		initTableInt(gtypeBasedTFTTable);
		initTableInt(memBasedYesManTable);
		initTableInt(memBasedTraitorTable);
		initTableInt(memBasedContraryTable);
		...
		while (exp < EXP) { // 協力への収束があった実験のみ記録をとる。
			// 各世代ごとにつかう一時的な記録配列の初期化
			initRecInt(memBasedTFT);
			initRecInt(gtypeBasedTFT);
			initRecInt(memBasedYesMan);
			initRecInt(memBasedTraitor);
			initRecInt(memBasedContrary);
			// 集団の生成
			...
			while (gen < GEN) {// 世代ループの始まり
				...
				memBasedYesMan[gen] = countMemBasedYesMan();
				memBasedTraitor[gen] = countMemBasedTraitor();
				memBasedContrary[gen] = countMemBasedContrary();
//				// check 出力させてみる
//				System.out.println(memBasedTFT[gen] + "\t" + memBasedYesMan[gen] + "\t" + memBasedTraitor[gen] + "\t"
//						+ memBasedContrary[gen]);
				//
				// 親リスト。
			...
			if (convergeFlag) {
				...
				for (int i = 0; i < aveTable.length; i++) {
					...
					memBasedTFTTable[i][exp] = memBasedTFT[i];
					memBasedYesManTable[i][exp] = memBasedYesMan[i];
					memBasedTraitorTable[i][exp] = memBasedTraitor[i];
					memBasedContraryTable[i][exp] = memBasedContrary[i];
				}
				// カウントを進める
				exp++;
			}
...
//					System.out.println(strMemory+":"+point+":\t"+tmpChrom);
	...
	// int 配列初期化メソッド
	private static void initRecInt(int[] in) {
		for (int i = 0; i < in.length; i++)
			in[i] = 0;
	}

	// int テーブル初期化メソッド
	private static void initTableInt(int[][] in) {
		for (int i = 0; i < in.length; i++)
			initRecInt(in[i]);
	}
